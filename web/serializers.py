...
from user.forms import USERNAME_PATTERN
from user.views import TEL_PATTERN
...
class RecruiterSimpleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Recruiter
        fields = ('recruiterid', 'name', 'tel', 'servstar')
...
class HouseInfoDetailSerializer(serializers.ModelSerializer):
    ...
    def get_photos(houseinfo):
        queryset = CompanyPhoto.objects.filter(company=houseinfo)
